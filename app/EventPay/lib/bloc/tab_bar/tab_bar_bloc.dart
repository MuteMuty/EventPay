...
class TabBarBloc extends Bloc<_TabBarEvent, TabBarState>
  ...
  FutureOr<void> _onResumed(
    ...
    if (state.index == 0 && homeTabNeedsRefresh()) {
      ...
      Navigator.popUntil(
          keys[0].currentContext!, ModalRoute.withName(EPRoute.root));

      Navigator.pushNamed(keys[0].currentContext!, EPRoute.tabNames[0]);
    }
  }
  ...
  void checkIndex(int index) {
    if (state.index == index || (index == 0 && homeTabNeedsRefresh())) {
      Navigator.popUntil(
          keys[index].currentContext!, ModalRoute.withName(EPRoute.root));

      Navigator.pushNamed(keys[index].currentContext!, EPRoute.tabNames[index]);
    } else {
